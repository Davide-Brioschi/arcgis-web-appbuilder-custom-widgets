...
define([
  ...
  'dojo/dom-style',
  './NlsStrings'
], function(_WidgetBase, declare, lang, array, html, domConstruct, on, keys, focusUtil, query, FilterManager,
  CheckBox, DropMenu, LoadingShelter, jimuUtils, PopupMenu, _TemplatedMixin, template, domClass, domStyle, NlsStrings) {
  ...
  return declare([_WidgetBase, _TemplatedMixin], {
    ...
    refresh: function() {
      ...
      // Remove Layer Filters
      array.forEach(this.operLayerInfos.getLayerInfoArray(), function(layerInfo) {
        this.filterManager.applyWidgetFilter(layerInfo.id, this.layerListWidget.id, "", null, null, false);
      }, this);
    ...
    addLegendNode: function(layerInfo, level, toTableNode) {
      ...
      try {
        ...
        domStyle.set(legendsNode, 'marginLeft', 38 + (level+1)*12 + 'px');
        //domStyle.set(legendsNode, 'font-size', (level + 1) * 12 + 'px');
        domStyle.set(legendsNode, 'font-size', '12px');
    ...
    redrawLegends: function(layerInfo) {
      ...
      if(legendsNode) {
        ...
        this.drawLegends(legendsNode, this.layerListWidget.appConfig.portalUrl);
      }
    },
        ...
        if (isFold) {
          ...
          if (layerInfo.isLeaf()) {
            ...
            if (legendsNode && loadingImg) {
              ...
              //customize
              var options = {
                 portalUrl: this.layerListWidget.appConfig.portalUrl,
                 widgetId: this.layerListWidget.id,
                 legendTableUrl: this.layerListWidget.config.legendTableUrl || null
              };
              layerInfo.layerListWidgetId = this.layerListWidget.id;
              if (layerInfo.isTiled || layerInfo.isTable || layerInfo.isBasemap()) {
                domStyle.set(loadingImg, 'display', 'none');
              }else{
                layerInfo.drawLegends(legendsNode, options);
              }
            }
          }
        }
