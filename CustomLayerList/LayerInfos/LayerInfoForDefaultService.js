...
define([
  ...
  './LayerInfoForDefault',
  './LayerObjectFacory'
], function(declare, array, lang, Deferred, topic, domConstruct, on, CheckBox,
   FilterManager, Query, QueryTask, LayerInfoForDefault, LayerObjectFacory) {
  return declare(LayerInfoForDefault, {
    ...
    drawLegends: function(legendsNode, options) {
      this._loadLegends(options.portalUrl).then(lang.hitch(this, function(legendInfo) {
        this._initLegendsNode(legendInfo, legendsNode, options);
      }));
    },

    _initLegendsNode: function(legendInfo, legendsNode, options) {
      var mapService = this.originOperLayer.mapService;
      ...
      if (legendInfo/*&& loadingImg*/) {
        ...
        if(legendLayer.legend.length == 1){
          this._defaultLegendsNode(legendLayer, legendsTable);
        }else{
          // layer name---legendLayer.layerName or this.title
          // get legend field
          if(options.legendTableUrl == null){
            this._defaultLegendsNode(legendLayer, legendsTable);
          }else{
            var queryTask = new QueryTask(options.legendTableUrl);
            var query = new Query();
            query.outFields = ["field_names", "field_types"];
            query.where = `layer_name='${legendLayer.layerName}'`;
            queryTask.execute(query).then(lang.hitch(this, function(response){
              if(response.features.length > 0){
                var legendFieldInfo = response.features[0].attributes;
                if(legendFieldInfo["field_names"] == null || legendFieldInfo["field_types"] == null){
                  this._defaultLegendsNode(legendLayer, legendsTable);
                }else{
                  this._customLayerLegendInfo = {
                      fieldInfo:{
                          names: [],
                          types: []
                      },
                  ...
                  var field_names = legendFieldInfo["field_names"].split(",");
                  var field_types = legendFieldInfo["field_types"].split(",");
                  array.forEach(field_names, function(field_name) {
                      this._customLayerLegendInfo.fieldInfo.names.push(field_name.trim());
                  }, this);
                  array.forEach(field_types, function(field_type) {
                      this._customLayerLegendInfo.fieldInfo.types.push(field_type.trim());
                  }, this);
                  array.forEach(legendLayer.legend, function(legend) {
                      this._customLayerLegendInfo.legendInfos.push({
                        values: legend.values || null,
                        checked: true
                      });
                  }, this);
                  array.forEach(legendLayer.legend, function(legend, idx) {
                      /*
                      ...
                      */
                      ...
                      var checkTd = domConstruct.create("td", {
                        "class": "legend-check-td",
                        "style": "padding-right: 10px"
                      }, legendTr);
                      
                      var ckSelect = new CheckBox({
                        checked: true, //layerInfo.visible
                        'class': "visible-checkbox-legend" + this.id,
                      });
            
                      domConstruct.place(ckSelect.domNode, checkTd);
                      ...
                      on(ckSelect.domNode, 'click', lang.hitch(this, this._onCkSelectLegendNodeClick, ckSelect, idx, options));
    ...
    _defaultLegendsNode: function(legendLayer, legendsTable){
        array.forEach(legendLayer.legend, function(legend) {
          /*
          if (legend.label === "<all other values>") {
            return;
    ...
    _onCkSelectLegendNodeClick: function(ckSelect, idx, options, evt){
        var widgetId = options.widgetId;
        var layerId = this.id;
        this._customLayerLegendInfo.legendInfos[idx].checked = ckSelect.checked;

        //layerInfo.isVisible()

        //Apply Filter
        var layerFilterExpr = "";
        var field_names = this._customLayerLegendInfo.fieldInfo["names"];
        var field_types = this._customLayerLegendInfo.fieldInfo["types"];
        var legendInfos = this._customLayerLegendInfo.legendInfos;
        var conditionExpr_1 = [], conditionExpr_2 = [];
        var otherFlag = false;
        array.forEach(legendInfos, function(legendInfo) {
            if(legendInfo.checked){
              if(legendInfo.values == null){
                otherFlag = true;
                var subExpr = [];
                for(var i=0;i<field_names.length;i++){
                    subExpr.push(`${field_names[i]} IS NULL`);
                }
               conditionExpr_1.push(`(${subExpr.join(" OR ")})`);
              }else{
                  array.forEach(legendInfo.values, function(value){
                    var sub_values = value.split(",");
                    var subExpr = [];
                    for(var i=0;i<field_names.length;i++){
                       if(field_types[i] == "String"){
                         subExpr.push(`${field_names[i]}='${sub_values[i]}'`);
                       }else{
                         subExpr.push(`${field_names[i]}=${sub_values[i]}`);
                       }
                    }
                    conditionExpr_1.push(`(${subExpr.join(" AND ")})`);
                  });
              }
            }
            if(legendInfo.values != null){
              array.forEach(legendInfo.values, function(value){
                ...
                conditionExpr_2.push(`(${subExpr.join(" OR ")})`);
              });
            }
        });
        if(conditionExpr_1.length > 0){
          if(otherFlag){
            layerFilterExpr = `${conditionExpr_1.join(" OR ")} OR (${conditionExpr_2.join(" AND ")})`;
          }else{
            layerFilterExpr = conditionExpr_1.join(" OR ");
          }
        }else{
          layerFilterExpr = "OBJECTID = -1";
        }
        
        var filterManager = FilterManager.getInstance(); 

        filterManager.applyWidgetFilter(layerId, widgetId, layerFilterExpr, true, null, false);
