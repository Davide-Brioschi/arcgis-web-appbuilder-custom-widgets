...
], function(declare, array, lang, Deferred, all, LayerInfo, gfx, domConstruct, on,
domAttr, domClass, aspect, portalUrlUtils, portalUtils, jimuUtils, jsonUtils, FilterManager, CheckBox, LabelLayer,
LabelClass, PopupTemplate, Legend, Graphic, Point, Query, RelationshipQuery, QueryTask) {
  var clazz = declare(LayerInfo, {
    ...
    drawLegends: function(legendsNode, options) {
      ...
      */
       // custom drawLegends
       this._customInitLegendsNode(legendsNode, options);
    },
    ...
    _customInitLegendsNode: function(legendsNode, options){
      var legendInfos = [];
      var layer = this.layerObject;
      ...
      if( this.layerObject && !this.layerObject.empty &&
        ...
        if (layer.renderer) {
            if((layer.renderer.authoringInfo && layer.renderer.visualVariables)  || layer.renderer.gradient || !layer.renderer.infos){
                domClass.remove(legendsNode, 'jimu-legends-div-flag');
                var layerInfos = [{
                  layer: this.layerObject
                }];
                ...
                var field = layer.getField(layer.renderer.attributeField);
                ...
                array.forEach(legendInfos, function(legendInfo, idx) {
                ...
                array.forEach(legendInfos, function(legendInfo, idx) {
                    ...
                    legendInfo.ckSelectDiv = domConstruct.create('div', {
                      'class': 'legend-select'
                    }, legendInfo.legendDiv);
                    ...
                    domConstruct.place(ckSelect.domNode, legendInfo.ckSelectDiv);
                    ...
                    on(ckSelect.domNode, 'click', lang.hitch(this,
                      this._onCkSelectLegendNodeClick, ckSelect, idx, options));
                }, this);
            }
        }
      }
    ...
    _onCkSelectLegendNodeClick: function(ckSelect, idx, options, evt){
        ...
        //Apply Filter
        var layerFilterExpr = "";
        var fieldInfo = this._customLayerLegendInfo.fieldInfo;
        var legendInfos = this._customLayerLegendInfo.legendInfos;
        var conditionExpr_1 = [], conditionExpr_2 = [];
        var otherFlag = false;
        array.forEach(legendInfos, function(legendInfo) {
            if(legendInfo.checked){
              if(legendInfo.value == null){
                conditionExpr_1.push(`${fieldInfo.name} IS NULL`);
                otherFlag = true;
              }else{
                if(fieldInfo.type == 'esriFieldTypeInteger' || fieldInfo.type == 'esriFieldTypeDouble' || fieldInfo.type == 'esriFieldTypeOID'){
                  conditionExpr_1.push(`${fieldInfo.name}=${legendInfo.value}`);
                }else{
                  conditionExpr_1.push(`${fieldInfo.name}='${legendInfo.value}'`);
                }
              }
            ...
            if(legendInfo.value != null){
              if(fieldInfo.type == 'esriFieldTypeInteger' || fieldInfo.type == 'esriFieldTypeDouble' || fieldInfo.type == 'esriFieldTypeOID'){
                conditionExpr_2.push(`${fieldInfo.name}<>${legendInfo.value}`);
              }else{
                conditionExpr_2.push(`${fieldInfo.name}<>'${legendInfo.value}'`);
              }
            }
