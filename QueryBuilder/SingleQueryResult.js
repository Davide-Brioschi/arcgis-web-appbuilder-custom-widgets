  ...
  function(declare, _WidgetBase, BindLabelPropsMixin, _TemplatedMixin, _WidgetsInTemplateMixin, Evented, template, lang,
    ...
    return declare([_WidgetBase, BindLabelPropsMixin, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      _addResultItems: function(features, resultLayer) {
        ...
        // new custom
        var items= query('.query-result-item', this.resultsTbody);
        var allCount = this.currentAttrs.query.allCount;
        var foundedNum = items.length;
        this._updateNumSpan(foundedNum, allCount);
      ...
      _createQueryResultItem: function(options, i) {
        ...
        //create PopupRenderer  new custom

        var strItem = [
          '<tr role="button" tabindex="0" class="jimu-table-row jimu-table-ow-separator query-result-item' + lastFeatureClass + '"  cellpadding="0" cellspacing="0">',
            '<td>',
                  ...
                  '<tr>',
                    '<td class="symbol-td"></td><td class="popup-td expanded">',
                        '<div class="popup-title-container">',
                          ...
                          '<div role="button" tabindex="0" aria-label="' + this.nls.collapse + '" ' + lastArrowData + ' class="popup-title-icon"></div>',
                          '<div role="button" tabindex="0" class="popup-title-delete-icon"></div>',
                        '</div>',
      ...
      _onResultsTableClicked: function(event) {
        ...
        if (html.hasClass(target, 'popup-title-delete-icon')) {
          ...
          var parent_node = tr.parentNode;
          
          //delete dom
          //tr.remove();
          parent_node.removeChild(tr);
          
          //delete feature
          var feature = tr.feature;
          var resultLayer = feature.getLayer();
          //var resultLayer = this.getResultLayer();
          resultLayer.remove(feature);
         
          // update numspan

          var allCount = this.currentAttrs.query.allCount;
          var foundedNum = parent_node.children.length; // parent.childElementCount
          this._updateNumSpan(foundedNum, allCount);
