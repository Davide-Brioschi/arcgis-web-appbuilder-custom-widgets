...
define([
  ...
  function(declare, array, html, lang, topic, Extent, SpatialReference,
    dojo, query, on, keys, focusUtil, _TemplatedMixin, _WidgetsInTemplateMixin, template,
    ...
    return declare([BaseWidget, _TemplatedMixin, _WidgetsInTemplateMixin], {
      ...
      toggleFilter: function(node, filterObj, parse) {
        ...
        this.filterManager.applyWidgetFilter(layerId, this.wId, layerFilterExpr, enableMapFilter, null, this.config.zoomto);
        ...
        if (!node.filterParams) {
          ...
          if(layerInfo){
            layerInfo.getLayerObject().then(lang.hitch(this, function(layerObject){
              ...
              // partsObj.wId = 'widgets_Filter_Widget' + this.wId + '_' + html.getAttr(node, 'data-index');
              var wid = this.isOnScreen ? ('widgets_Filter_Widget' + this.wId) : this.wId;
      ...
      _onCustomFilterToggle: function(isChecked){
        ...
        if(!isChecked){
          this.filterManager.applyWidgetFilter(this.selectedLayer.id, this.wId + '-custom-filter', '1=1', true, null, this.config.zoomto);
      ...
      _applyCustomFilter: function(){
        ...
        this.filterManager.applyWidgetFilter(this.selectedLayer.id, this.wId + '-custom-filter', filterJson.expr, true, null, this.config.zoomto);
      ...
      _unapplyFilterByStore: function(){
        if (this._store) {
          for (var p in this._store) {
            if (p) {
              this.filterManager.applyWidgetFilter(p, this.wId, "", null, null, this.config.zoomto);
            }
          }
        }
      },
