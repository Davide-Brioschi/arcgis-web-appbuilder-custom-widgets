  ...
  function(declare, _WidgetBase, BindLabelPropsMixin, _TemplatedMixin, _WidgetsInTemplateMixin, Evented, template,
    ...
    var clazz = declare([_WidgetBase, BindLabelPropsMixin, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
      ...
      _initSelf: function(){
        ...
        partsObj.wId = 'widgets_Query_Widget' + this.wId + '_' + this.wId;
        ...
        if(spatialFilter.useFeatures){
          ...
          //var layerInfoArray = this.layerInfosObj.getLayerInfoArray();
          var layerInfoArray = this.layerInfosObj.getLayerInfoArrayOfWebmap();
          if(spatialFilter.useFeatures.selectLayerId){
            array.forEach(layerInfoArray, function(layerInfo) {
              var layer = layerInfo.layerObject;
              if (layer.id != spatialFilter.useFeatures.selectLayerId) {
                ignoredFeaturelayerIds.push(layer.id);
              }
            }, this);
          }else{
          ...
          // new custom
          if(spatialFilter.useFeatures.selectLayerId){
            var layerInfo = this.layerInfosObj.getLayerInfoById(spatialFilter.useFeatures.selectLayerId);
            var selectLayer = layerInfo.layerObject;
            this.spatialFilterByFeatures.setSelectedLayer(selectLayer);
          }
