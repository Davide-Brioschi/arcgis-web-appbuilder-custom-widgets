...
define([
  ...
  './setting/filterUtils',
  'jimu/utils',
  'jimu/dijit/_SingleFilterParameter',
      ...
      */
      ...
      _getNewValidPartsObj: function(_partsObj, returnNullIfInvalidPart){
        ...
        var tryPushParts = lang.hitch(this, function(p){
          var statusArr = [];
          p.parts = array.filter(p.parts, lang.hitch(this, function(subPart){
            if(subPart.parts){
               subPart.parts = array.filter(subPart.parts, lang.hitch(this, function(singlePart){
                  var status = validateSinglePart(singlePart);
                  return status > 0;
               }));
               if(subPart.parts.length > 1){
              ...
              var status = validateSinglePart(subPart);
              statusArr.push(status);
              return status > 0;
            }
          }));
        ...
        if(cascade === "previous"){
          for(i = 0; i < partsObj.parts.length; i++){
            ...
            if(p.majorCascadeIndex < desPart.majorCascadeIndex){
              ...
              p2 = lang.clone(p);
              p2.parts = array.filter(p2.parts, lang.hitch(this, function(subPart){
                if(subPart.parts){
                   subPart.parts = array.filter(subPart.parts, lang.hitch(this, function(singlePart){
                  ...
                  return subPart.minorCascadeIndex < desPart.minorCascadeIndex;
                }
              }));
        ...
        array.forEach(partsObj, lang.hitch(this, function(item){
          if(item.parts){
            array.forEach(item.parts, lang.hitch(this, function(item2){
              if(item2.parts){
                array.forEach(item2.parts, lang.hitch(this, function(item3){
                  if(item3.interactiveObj && item3.interactiveObj.cascade === true){
                    item3.interactiveObj.cascade = "previous";
                  }else if(item3.interactiveObj.cascade === false){
                    item3.interactiveObj.cascade = "none";
                  }
                }));
              }else{
        ...
        for(var i = 0; i < partsObj.parts.length; i++){
          ...
          if(p.parts){
            for(var j = 0; j < p.parts.length; j++){
              var p2 = p.parts[j];
              if(p2.parts){
                  for(var k = 0; k < p2.parts.length; k++){
                    var p3 = p2.parts[k];
                    p3.majorCascadeIndex = p2.majorCascadeIndex;
                    if(p2.logicalOperator === 'AND'){
                      p3.minorCascadeIndex = k;
                    }else{
                      p3.minorCascadeIndex = 0;
                    }
                    p3.cascadeIndex = parseFloat(p3.majorCascadeIndex + "." + p3.minorCascadeIndex);
                  }
              }else{
