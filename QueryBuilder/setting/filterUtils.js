...
function(declare, lang, array, locale, dojoNumber, esriLang, ItemFileWriteStore, jimuUtils, moment) {
  ...
  var clazz = declare([], {
          ...
          isValidPartsObj = array.every(partsObj.parts, lang.hitch(this, function(part){
            if(part.parts){
              if(part.parts.length > 0){
                return array.every(part.parts, lang.hitch(this, function(subPart){
                  if(subPart.parts){
                    if(subPart.parts.length > 0){
                      return array.every(subPart.parts, lang.hitch(this, function(singlePart){
    ...
    getExprByFilterObj: function(partsObj) {
      ...
      var subPart, isChecked;
      if(partsObj.parts.length === 0){
        filterString = "1=1";
        displaySQL = "1=1";
      }else if(partsObj.parts.length === 1) {
          subPart = partsObj.parts[0];
          if(subPart.valueObj && lang.isArray(subPart.valueObj.value) && subPart.valueObj.type !== 'multiple'){
            isChecked = this._checkIfValObjArrayAndChecked(subPart.valueObj.value);
            if(isChecked){
              filterString = this.builtFilterString(subPart);
              displaySQL = subPart.displaySQL;
            }else{
              filterString = "1=1";
              displaySQL = "1=1";
            }
          ...
          for (var i = 0; i < partsObj.parts.length; i++) {
            subPart = partsObj.parts[i];
            var str, str2;
            if(subPart.valueObj && lang.isArray(subPart.valueObj.value) && subPart.valueObj.type !== 'multiple'){
              isChecked = this._checkIfValObjArrayAndChecked(subPart.valueObj.value);
              if(isChecked){
                str = this.builtFilterString(subPart);
                str2 = subPart.displaySQL;
              }else{
                str = "1=1";
                str2 = "1=1";
              }
    ...
    _handleVirtualDate: function(partsObj){
      ...
      array.forEach(partsObj.parts, lang.hitch(this, function(part){
        if(part.parts){
          array.forEach(part.parts, lang.hitch(this, function(subPart){
            if(subPart.parts){
              array.forEach(subPart.parts, lang.hitch(this, function(singlePart){
                 this._updateRealDateByVirtualDate(singlePart);
              }));
            }else{
              this._updateRealDateByVirtualDate(subPart);
            }
          }));
        }else{
      ...
      var isReady = array.every(partsObj.parts, lang.hitch(this, function(part){
        ...
        if(part.parts){
          result = array.every(part.parts, lang.hitch(this, function(subPart){
            ...
            if(subPart.parts){
               return array.every(subPart.parts, lang.hitch(this, function(singlePart){
                 return this._isPartReadyToBuild(singlePart);
               }));
            }else{
              return this._isPartReadyToBuild(subPart);
            }
    ...
    builtFilterString: function(partsObj) {
      ...
      if (partsObj.parts) {
        ...
        var join = "";
        for (var i = 0; i < partsObj.parts.length; i++) {
          var subPart = partsObj.parts[i];
          if(subPart.parts){
            sub_join = "", str = "", str2 = "";
            for(var j=0; j < subPart.parts.length; j++){
               var singlePart = subPart.parts[j];
               var obj = this.builtSingleFilterString(singlePart);
               singlePart.expr = obj.whereClause;  //displaySQL exists in part already
               if (!esriLang.isDefined(obj.whereClause)) {
                 // we're missing input
                 return null;
               }
               str += sub_join + obj.whereClause;
               str2 += sub_join + singlePart.displaySQL;
               sub_join = " " + subPart.logicalOperator + " ";
            }
            filterString += join + "(" + str + ")";
            displaySQL += join + "(" + str2 + ")";
            ...
            var obj = this.builtSingleFilterString(subPart);
            subPart.expr = obj.whereClause;  //displaySQL exists in part already
            if (!esriLang.isDefined(obj.whereClause)) {
              // we're missing input
              return null;
            }
            filterString += join + obj.whereClause;
            displaySQL += join + subPart.displaySQL;
            join = " " + partsObj.logicalOperator + " ";
          }
        }
      } else {
    ...
    getFilterObjByExpr: function(defExpr){
      ...
      array.forEach(partsObj.parts, function(part){
        ...
        // 3 levels
        array.forEach(subPartsObj.parts, function(singlePart){
            singlePart.expr = singlePart.expr.trim();
            if (singlePart.expr.startsWith('(') && (singlePart.expr.search(/\)$/) > -1)) {
              
              var str = singlePart.expr.substring(1, singlePart.expr.length - 1);
              var pos1 = str.indexOf('(');
              var pos2 = str.indexOf(')');
              if ((pos1 === -1 && pos2 === -1) || pos1 < pos2) {
                singlePart.expr = str;
            ...
            var singlePartsObj = this.findParts(singlePart.expr, "AND");
            if (singlePartsObj.parts.length === 1) {
              singlePartsObj = this.findParts(singlePart.expr, "OR");
            }
            if (singlePartsObj.parts.length > 1) {
              singlePart.parts = singlePartsObj.parts;
              singlePart.logicalOperator = singlePartsObj.logicalOperator;
            }
        }, this);
      }, this);
          ...
          array.forEach(partsObj.parts, lang.hitch(this, function(partOrParts) {
            if (partOrParts) {
              if (partOrParts.parts) {
                ...
                array.forEach(partOrParts.parts, lang.hitch(this, function(subPart) {
                  if(subPart.parts){
                    array.forEach(subPart.parts, lang.hitch(this, function(singlePart) {
                        //parse numbers
                    ...
                    //parse numbers
                    this._handleParsedValuesForSinglePart(subPart);
                    //add 'value' type if value type is missing
                    this._addDefalutValueTypeForSinglePart(subPart);
                  }
                }));
              } else {
  ...
  clazz.isAskForValues = function(partsObj){
    ...
    result = array.some(parts, function(item) {
      if (item.parts) {
        return array.some(item.parts, function(subPart) {
          if(subPart.parts){
            return array.some(subPart.parts, function(singlePart) {
              return !!singlePart.interactiveObj;
            });   
          }else{
            return !!subPart.interactiveObj;
          }
        });
      } else {
    ...
    result = array.some(parts, function(item) {
      if (item.parts) {
        return array.some(item.parts, function(subPart) {
          if(subPart.parts){
            return array.some(subPart.parts, function(singlePart) {
              return !!singlePart.valueObj.virtualDate || !!singlePart.valueObj.virtualDate1 || !!singlePart.valueObj.virtualDate2;
            });
          }else{
            return !!subPart.valueObj.virtualDate || !!subPart.valueObj.virtualDate1 || !!subPart.valueObj.virtualDate2;
          }
        });
      } else {
