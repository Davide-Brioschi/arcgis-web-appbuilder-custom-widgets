...
function(declare, lang, array, html, on, query, Deferred, Evented, all, BaseWidget, _TemplatedMixin, _WidgetsInTemplateMixin, template,
  TabContainer, LoadingIndicator, Message, utils, zoomToUtils, LayerInfos, editorManager,
  resultRendererManager, GPMessage, Geoprocessor, JobInfo, ImageParameters, esriRequest, gputils) {
  var clazz = declare([BaseWidget, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
    ...
    startup: function(){
      ...
      this.tab = new TabContainer({
        ...
        nls: this.nls,  // new custom  
        hideNonQueriedLayers: this.config.hideNonQueriedLayers,  // new custom
      });
    ...
    onClose: function(){
      // new custom
      if(this.config.hideNonQueriedLayers){
        if(this.tab._currentCtrlNode.label == this.nls.output){ // or .textContent
          var layerInfosObj = LayerInfos.getInstanceSync();
          //var layerInfoArray = layerInfosObj.getLayerInfoArray();
          var layerInfoArray = layerInfosObj.getLayerInfoArrayOfWebmap();
          array.forEach(layerInfoArray, function(layerInfo) {
              if(layerInfo.nonQueriedVisible){
    ...
    onOpen: function(){
      ...
      if(this.config.hideNonQueriedLayers){
        if(this.tab._currentCtrlNode.label == this.nls.output){ // or .textContent
          ...
          var layerInfoArray = layerInfosObj.getLayerInfoArrayOfWebmap();
          array.forEach(layerInfoArray, function(layerInfo) {
              layerInfo.nonQueriedVisible = layerInfo.isVisible();
    ...
    _generateUniqueID: function(){
      this.uniqueID = this.wId.replace(/[\/\.]/g, '_');
    },
