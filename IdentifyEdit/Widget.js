...
define([
  ...
  function(declare, lang, array, html, esriBundle, on, query, Json, Deferred, aspect, all,
    ...
    jimuUtils, portalUrlUtils, portalUtils, SelectionManager, FilterManager, Role, Editor, editingUtil, Popup, TemplatePicker,
    Extent, Point, rendererJsonUtils, Graphic, domUtils, EsriQuery, QueryTask, Button, editUtils, FilterEditor,
    RelatedRecordsEditor) {
    var Clazz = declare([BaseWidget, _WidgetsInTemplateMixin], {
      name: 'IdentifyEdit',
      baseClass: 'jimu-widget-identify-edit',
      editor: null,
      _defaultStartStr: "",
      _defaultAddPointStr: "",
      ...
      startup: function() {
        ...
        this.editPopup = new Clazz.EditPopup(null, html.create("div",
                                                    {"class":"jimu-widget-identify-edit-infoWindow"},
                                                    null,
                                                    this.map.root));
        ...
        // custom
        if(!this.config.editor.showPanel){
          //html.setStyle(this.id + '_panel','display','none');
          //document.getElementById(this.id + '_panel').classList.add('identify-edit-widget-panel');
          html.addClass(this.id + '_panel', 'identify-edit-widget-panel')
        }
        this._setConfigEditor();
      },
     
      _setConfigEditor: function(){
          var configEditor = this.config.editor;
          //console.log(configEditor);
          if(this.config.editor.blockedUrl){
              this.portal = portalUtils.getPortal(this.appConfig.portalUrl);
              var user_name = this.portal.user.username;

              var queryTask = new QueryTask(this.config.editor.blockedUrl);
              var query = new EsriQuery();
              query.outFields = ["layer_name","field_names","definition_query"];
                  ...
                  if(response.features.length > 0){
                      ...
                      array.forEach(features, function(feature){
                        if(feature.attributes["field_names"]==null){
                          blocked_fields.push({
                            layer_name: feature.attributes["layer_name"],
                            field_names: []
                        });
                        }else{
                          blocked_fields.push({
                              layer_name: feature.attributes["layer_name"],
                              field_names: feature.attributes["field_names"].split(",").map(item=>item.trim())
                          });
                        }
                      });
                      array.forEach(features, function(feature){
                        if(feature.attributes["definition_query"] != null){
                            filter_array.push({
                              layer_name: feature.attributes["layer_name"],
                              definition_query: feature.attributes["definition_query"]
                            });
                        }
                      });
                      if(configEditor.layerInfos){
                        /*
                        configEditor.layerInfos.map(function(layerInfo){
                           layerInfo.allowCreate = false;
                           layerInfo.allowDelete = false;
                           layerInfo.disableGeometryUpdate = true;
                           return layerInfo;
                        });
                        */
                        configEditor.layerInfos.map(lang.hitch(this,function(layerInfo){
                          for(var i=0;i<blocked_fields.length;i++){
                              if(layerInfo.featureLayer.id == this._getLayerId(blocked_fields[i].layer_name)){
                                  // fieldInfos
                                  if(blocked_fields[i].field_names.length == 0){
                                      for(var j=0;j<layerInfo.fieldInfos.length;j++){
                                        layerInfo.fieldInfos[j].isEditable = false;
                                      }
                                  }else{
                                      for(var j=0;j<layerInfo.fieldInfos.length;j++){
                                        if(layerInfo.fieldInfos[j].isEditable == true){
                                          for(var k=0;k<blocked_fields[i].field_names.length;k++){
                                              if(layerInfo.fieldInfos[j].fieldName == blocked_fields[i].field_names[k]){
                                                layerInfo.fieldInfos[j].isEditable = false;
                                                break;
                                              }
                                          }
                        ...
                        this._configEditor = lang.clone(configEditor);
      ...
      _onActive: function(){
        if(this._hasEditPrivilege) {
          ...
          if(this._configEditor.usingSaveButton) {
            //this._originalClosePanel = lang.hitch(this._panelManager, this._panelManager.closePanel);
            this._originalClosePanel = this._panelManager.closePanel;
            this._panelManager.closePanel = this._wrapExistFunctionWithConfirmDialog(this._panelManager,
                                                                                     this._originalClosePanel);
          }
      ...
      onDeActive: function(){
        //this._onDeActive();
        this._panelManager.closePanel(this.id + '_panel');
      ...
      _onDeActive: function(){
          if(this._hasEditPrivilege) {
            ...
            if(this._configEditor.usingSaveButton) {
              this._panelManager.closePanel = this._originalClosePanel;
            }
      ...
      _getLayerInfosInConfig: function() {
        ...
        if(this._configEditor.honorSettingOfWebmap) {
          ...
          if(this._configEditor.blockedUrl){
            if(!this._configEditor.layerInfos || this._configEditor.layerInfos.length == 0){
              layerInfos = [];
        ...
        } else if(!this._configEditor.layerInfos || this._configEditor.layerInfos.length == 0) {
